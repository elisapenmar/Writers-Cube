...
function CharacterCard({
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-md p-3 group">
      <div className="flex items-center gap-2">
        <input
          value={name}
          ...
          placeholder="Name"
          className="flex-1 bg-transparent border-0 outline-none font-serif text-base text-zinc-900 px-0"
        />
        <input
          value={role}
          ...
          placeholder="role"
          className="w-32 bg-transparent border-0 outline-none text-xs text-zinc-500 px-0 text-right"
        />
        <button
          onClick={handleDelete}
          className="text-xs text-zinc-300 hover:text-red-700 opacity-0 group-hover:opacity-100 shrink-0"
          title="Delete"
        >
          ×
        </button>
      ...
      {editingDescription ? (
        <textarea
          ...
          className="w-full mt-2 bg-zinc-50 border border-zinc-200 rounded px-2 py-1.5 text-sm font-serif leading-relaxed outline-none focus:border-zinc-400"
          placeholder="Description, traits, arc, voice…"
        />
      ) : (
        <div
          onClick={() => setEditingDescription(true)}
          className="mt-1 cursor-text text-sm text-zinc-700 font-serif leading-relaxed whitespace-pre-wrap min-h-[1.2em] hover:bg-zinc-50 rounded px-1 -mx-1"
        >
          {description || (
            <span className="italic text-zinc-400">Description, traits, arc, voice…</span>
          )}
        </div>
      )}

      {savingDescription && (
        <div className="text-[10px] text-zinc-400 mt-1">Saving…</div>
      )}
    </div>
  );
}
