...
import type { ElementMatrix } from "@/server/story-element-crud";
...
/**
 * Generic element × chapter appearance grid (places, items). Mirrors
 * CharacterGrid but takes its data loader and the row-header label as props.
 */
  ...
  if (matrix.chapters.length === 0 || matrix.rows.length === 0) {
    return (
      <p className="text-xs text-[var(--wc-faint)] px-1 py-2">
        Add {nounLabel.toLowerCase()}s and write some chapters to see where each one appears.
      </p>
    );
  }
